...
<div class="page-banner">
  <div class="container">
    <h1>Sanctuary Events & Festivals</h1>
    <div class="breadcrumb">
      <a href="<?= $baseUrl ?>index.php">Home</a> / <span>Events</span>
    </div>
  </div>
</div>
  ...
  <div class="container">
    ...
    <form method="GET" action="events.php" class="filter-bar">
      <div class="filter-buttons">
        <a href="events.php" class="filter-btn <?= empty($statusFilter) ? 'active' : '' ?>">All Events</a>
        <a href="events.php?status=Upcoming" class="filter-btn <?= $statusFilter === 'Upcoming' ? 'active' : '' ?>">Upcoming</a>
        <a href="events.php?status=Ongoing" class="filter-btn <?= $statusFilter === 'Ongoing' ? 'active' : '' ?>">Ongoing</a>
        <a href="events.php?status=Completed" class="filter-btn <?= $statusFilter === 'Completed' ? 'active' : '' ?>">Completed</a>
      </div>
        ...
        <?php foreach ($events as $event): ?>
          <div class="card">
            ...
            <div class="card-body">
              ...
              <p class="card-text"><?= htmlspecialchars(mb_strimwidth($event['description'], 0, 110, '...')) ?></p>
              
              <div class="card-actions" style="margin-top:auto;">
                <a href="event-details.php?id=<?= $event['id'] ?>" class="btn btn-primary btn-sm btn-block">View Details & Attend 🗓</a>
              </div>
            </div>
          </div>
        <?php endforeach; ?>
    ...
    <?php else: ?>
      <div class="text-center" style="padding:60px; background:white; border-radius:var(--radius-md);">
        ...
        <p style="color:var(--text-muted); margin-bottom:20px;">Try adjusting your search query or filter.</p>
